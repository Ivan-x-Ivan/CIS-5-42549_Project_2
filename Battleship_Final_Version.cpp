...
int main(int argv, char **argc){
    ...
    accry[games] = round((10.0f / atemps) * 100.0f);
    gmrslts(shotLog, sunkShp, 4, numSunk); //display the map of the game
    acrcyOF << fixed << setprecision(2) << games+1 << " " << accry[games] << " " << atemps << endl;
    ...
    cout << "Leaderboard" << endl << "-----------------------------------------------------------------------------------------------------------------" << endl;//display the header of the leaderboard
    for (int i=0; acrcyIF>>game>>accrcy>>atemps; i++){
        cout << endl << endl << endl;//display the header for the leaderboard
...
void gmrslts(vector<string> &log, char sunk[][2], int wdth, int numSnk){
    ...
    cout << "   |   o = unknown   * = miss   x = hit/sunk" << endl << endl;
}
