...
bool duplHit(char, char, char[], char[],int sze); //Returns true if player coordinate matches that of an already sunk ship
...
int main(int argv, char **argc){
    ...
    char shipCrd[5][MAXSHPS]; //2d array storing the coordinates of the randomly generated ships
...
bool duplHit(char uC, char uR, char clmn[], char row[], int sze){ //Returns true if the player coordinates match those of an already sunk ship. This prevents duplicate ship sunk counters
    ...
    for (int i=0; i<sze; i++){
        ...
        //Check if user's row value matches that of one of the sunk ship's row values
        if(row[i] == uR){
            isRow = true;
        }
