...
bool duplHit(char, char, char[][],int sze); //Returns true if player coordinate matches that of an already sunk ship
...
int main(int argv, char **argc){
    ...
    //Holds the coordinate of the sunk ships as a 2d array. Row represents each ship, column represents each coordinate with 2 coordinates
    char snkShps[MXSHPS][MXCRDS] = {}; //Holds the coordinates for every sunk ship//Set every value in the 2d-array to 0
...
void filCrds(char shipC[], char shipR[], int sze){ //takes in the column and row array of the ships
...
bool duplHit(char uC, char uR, char clmn[], char row[], int sze){ //Returns true if the player coordinates match those of an already sunk ship. This prevents duplicate ship sunk counters
    for (int i=0; i<sze; i++){
        //Check if user's column value matches that of one of the sunk ship's column values
        if (clmn[i] == uC && row[i] == uR){
            return true;
        }
    }
    return false;
...
bool chckBnd(char cCrds, char rCrds){ //Retruns true if player coordinate is out bounds from the grid
    if ((cCrds < 97 || cCrds > 106) || (rCrds < 48 || rCrds > 57)){
        return true; //Returns true if both column and row values are not in the range of the values accepted by their ascii value
    }
    else{
        return false;
    }
